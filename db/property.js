var db_connection_pool = require('./connectionpool.js').connectionPool;
...
var retrievePropertyDetails = async function(propertyid,callback){
    /*
    const query = "SELECT * FROM "+DB_TABLE_PROPERTY+" WHERE "+DB_COLUMN_PROPERTY_ID+"=?";
    console.log(query)
    db_connection_pool.query(query,[id],function(error,results,fields){
        console.log(results);
        callback(error,results,fields);
    });
    */
...
var fetchUserMessages = async function(userid){
    try{
        ...
        msg.forEach((message)=>{
         ...
         console.log("properties before its loop");
         console.log(properties);
         properties.forEach((property)=>{

            msg.forEach((message)=>{
                if(message.propertyid == property.propertyid){
                    console.log(msg.propertyid,property.propertyid);
